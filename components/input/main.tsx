...
import { useThemeMode } from '@providers/hook';
import { ActiveIconProp, TextBoxProps } from "./interface";
...
export interface PrimaryInputProps extends TextInputProps, TextBoxProps {
...
export const PrimaryInput: React.FC<PrimaryInputProps> = ({
  ...
  return (
    <View style={{ flexGrow: 1 }}>
      {label ? (
        <Label color={colors.black} {...labelProps}>
          {label}
        </Label>
      ) : null}
      <InputWrapper
      ...
      {(bottomText && !bottomTextOnError) ||
        ...
        <BottomText color={inputError ? colors.green : colors.black}>
          {bottomText}
        </BottomText>
      ) : null}
    </View>
